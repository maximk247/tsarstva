...
**Поиск.** Основная выдача поиска открывается inline в центральной области ридера, а не уводит пользователя на отдельную страницу. `/search` сохраняется как прямой отдельный маршрут. Клиентский поиск в ридере читает статический `frontend/public/search-index.json`, который генерируется перед `dev`/`build` скриптом фронтенда.
...
- При точечных визуальных правках по скриншоту меняй только указанную поверхность/состояние. Не меняй палитру, глобальные токены или соседние компоненты, если пользователь просит только шрифт, цвет выделения, отступы или радиус конкретного элемента.
