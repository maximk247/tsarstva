import type { CrossRef, ManualEntry } from "@/types/bible";

import manualRaw from "@/data/cross-refs/manual.json";

const manualData = manualRaw as { refs: ManualEntry[] };

// Build manual index at module load time
const manualIndex: Record<string, CrossRef[]> = {};

for (const entry of manualData.refs) {
  const key = entry.from; // "1kgs:1:1"
